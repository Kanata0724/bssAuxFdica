% ======================================================================
% 学習用・日本語解説付きコピー
% 元ファイル: bss_eval/bss_decomp_gain.m
...
% [解説 L28] 値 nargin に応じて、入力形式・音源モデル・評価モードなどの処理経路を切り替えます。
switch nargin
% [解説 L29] 指定された選択値に対応する方式を実行し、同じ関数内で複数のアルゴリズムや設定を切り替えます。
    case 3
% [解説 L30] この関数の複数の計算結果を受け取り、varargout、varargout、varargout にそれぞれ保存します。
...
% [解説 L31] 指定された選択値に対応する方式を実行し、同じ関数内で複数のアルゴリズムや設定を切り替えます。
    case 4
% [解説 L32] この関数の複数の計算結果を受け取り、varargout、varargout、varargout、varargout にそれぞれ保存します。
% [意図] 一つの計算で得られる関連結果を分けて保持し、後続の評価・更新・復元でそれぞれ利用するためです。
        [varargout{1},varargout{2},varargout{3},varargout{4}]=bss_decomp_filt(varargin{1},varargin{2},varargin{3},varargin{4},0);
% [解説 L33] 既知の選択肢に当てはまらない入力を処理し、想定外の設定を見逃さないようにします。
    otherwise
